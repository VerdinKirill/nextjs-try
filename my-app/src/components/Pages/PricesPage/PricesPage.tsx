...
import {CalcPricesModal} from './CalcPricesModal';
import {CalcUnitEconomyModal} from './CalcUnitEconomyModal';
...
export const PricesPage = () => {
    ...
    const handleSendButton = () => {
        ...
        for (let i = 0; i < filteredData.length; i++) {
            ...
            if (nmId && wbPrice && rozPrice >= primeCost) {
                ...
                doc.pricesData[art]['fixPrices'] = undefined;
            }
        }
    ...
    const fixedPriceRender = (args: any, keys: any, defaultRenderFunctionRes: any) => {
        const {row} = args;
    ...
    const updateFixPrices = async (params: any) => {
        try {
            console.log(params);
    ...
    const [lastCalcOldData, setLastCalcOldData] = useState<any>({});

    const [doc, setDoc] = useState<any>(
        undefined as undefined | {pricesData: {}; fixArtPrices: {}},
    );
    ...
    const filterTableData = (withfFilters: any = {}, tableData: any = {}) => {
        ...
        for (const [key, val] of Object.entries(filteredSummaryTemp)) {
            ...
            filteredSummaryTemp[key] = getRoundValue(val as any, temp.length);
        }
